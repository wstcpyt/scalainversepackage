...
object InverseSparkMain {
  ...
  private val randMax = 0.2d
  private val cglsIndex = 7

  def main(args: Array[String]): Unit = {
    val conf = new SparkConf().setAppName("Inverse Problem")
    val sc = new SparkContext(conf)
    val staticPopulation = CEAlgorithm.initPopulation(populationLength, 3, randMin, randMax)
    var subPopulationbList = SubPopulationList.generateList(staticPopulation, threadNumber, populationLength, populationLengthPerThread)
    0 until loop foreach{_ =>
      val subPopulationListRDD: RDD[SubPopulation] = sc.parallelize(subPopulationbList)
